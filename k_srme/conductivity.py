...
def init_phono3py(
    ...
    **kwargs
    ...
    if "fc2_supercell" not in atoms.info.keys() :
        raise ValueError(f'{atoms.get_chemical_formula(mode="metal")} "fc2_supercell" was not found in atoms.info when calculating force sets.')
    

    if "fc3_supercell" not in atoms.info.keys() :
        raise ValueError(f'{atoms.get_chemical_formula(mode="metal")} "fc3_supercell" was not found in atoms.info when calculating force sets.')
    
    
    if "fc3_supercell" not in atoms.info.keys() :
        raise ValueError(f'{atoms.get_chemical_formula(mode="metal")} "q_mesh" was not found in atoms.info when calculating force sets.')
...
def get_fc2_and_freqs(
    ...
    if calculator is None :
        raise ValueError(f'{get_chemical_formula(ph3)} "calculator" was provided when calculating fc2 force sets.')
...
def get_fc3(
    ...
    if calculator is None :
        raise ValueError(f'{get_chemical_formula(ph3)} "calculator" was provided when calculating fc3 force sets.')
...
def calculate_conductivity(
    ...
    try:
        mode_kappa_TOT = calculate_mode_kappa_TOT(deepcopy(cond.mode_kappa_P_RTA[0]),deepcopy(cond.mode_kappa_C[0]),deepcopy(cond.mode_heat_capacities))    
    except AttributeError as exc:
        warnings.warn(f"Calculate mode kappa tot failed in {get_chemical_formula(ph3)}: {exc}")

    kappa_dict["mode_kappa_TOT"] = mode_kappa_TOT

    sum_mode_kappa_TOT = mode_kappa_TOT.sum(axis = tuple(range(1,mode_kappa_TOT.ndim-1)))/np.sum(kappa_dict["weights"])

    if np.all((sum_mode_kappa_TOT - kappa_dict["kappa_P_RTA"]) <= MODE_KAPPA_THRESHOLD) :
        warnings.warn(f"Total mode kappa does not sum to total kappa. mode_kappa_TOT sum : {sum_mode_kappa_TOT}, kappa_TOT_RTA : {kappa_dict['kappa_P_RTA']}")
