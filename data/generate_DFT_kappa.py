...
symm_no_name_map = {225: "rocksalt", 186: "wurtzite", 216: "zincblende"}

pbar = True

tar_dir = (
    "/mnt/scratch2/q13camb_scratch/bp443/foundational_TC/release/phonondb-PBE-data/"
)
tar_list = [
    "phono3py_params_RS.tar.xz",
    "phono3py_params_WZ.tar.xz",
    "phono3py_params_ZB.tar.xz",
]
...
df_mp_id = pd.read_csv(
    mp_phonondb_id_file, sep=" ", header=None, names=["name", "symm.no", "mp-id"]
)
element_pattern = r"([A-Z][a-z]?)"
df_mp_id["element_list"] = df_mp_id["name"].apply(
    lambda x: sorted(re.findall(element_pattern, x))
)
...
def get_mat_info(atoms):
    ...
    compound = df_mp_id[
        df_mp_id["element_list"].apply(lambda x: np.all(x == list_chemicals))
    ]

    structure = compound[
        compound["symm.no"].apply(lambda x: int(x))
        == int(get_spacegroup(atoms, symprec=1e-5).no)
    ]
...
for tar_filename in tqdm(tar_list, desc="Tar files", disable=not pbar):
    with tarfile.open(tar_dir + tar_filename, "r:xz") as tar:
        ...
        for file_to_read in tqdm(
            files,
            leave=False,
            desc=f"Reading yaml files in tar file {tar_filename}",
            disable=not pbar,
        ):
