...
# EDITABLE CONFIG
model_name = "MACE"
checkpoint = "small"#"https://github.com/ACEsuit/mace-mp/releases/download/mace_mp_0/2024-01-07-mace-128-L2_epoch-199.model" #"https://github.com/ACEsuit/mace-mp/releases/download/mace_mp_0b/mace_agnesi_medium.model"
...
slurm_array_task_count = int(os.getenv("K_SRME_RESTART_ARRAY_TASK_COUNT",os.getenv("SLURM_ARRAY_TASK_COUNT", "1")))
slurm_array_task_id = int(os.getenv("SLURM_ARRAY_TASK_ID", "0"))
slurm_array_job_id = os.getenv("SLURM_ARRAY_JOB_ID", os.getenv("SLURM_JOB_ID", "debug"))
slurm_array_task_min = int(os.getenv("K_SRME_RESTART_ARRAY_TASK_MIN",os.getenv("SLURM_ARRAY_TASK_MIN", "0")))
...
out_path = f"{out_dir}/conductivity_{slurm_array_job_id}-{slurm_array_task_id:>03}.json.gz"
...
if slurm_array_job_id == "debug":
    ...
    atoms_list =atoms_list[slurm_array_task_id - slurm_array_task_min::slurm_array_task_count] 
...
for atoms in tqdm_bar:
    ...
    info_dict = {"desc":mat_desc,"name": mat_name, "initial_space_group_number": atoms.info["symm.no"],"errors":[],"error_traceback":[]}
    ...
    try:
        ...
        if max_steps > 0:
            if not if_two_stage_relax:
                ...
                optimizer = optim_cls(filtered_atoms,logfile=f'{out_dir}/relax_{slurm_array_task_id}.log')
                optimizer.run(fmax=force_max, steps=max_steps)

                reached_max_steps = False
                if optimizer.step == max_steps:
                    reached_max_steps= True
                    print(f"Material {mat_desc=}, {mat_id=} reached max step {max_steps=} during relaxation.")
                ...
                relax_dict = {"structure" : aseatoms2str(atoms),
                    'max_stress': max_stress,
                    'reached_max_steps':reached_max_steps,
                    'relaxed_space_group_number':symm_no,
                    'broken_symmetry': symm_no != init_info["initial_space_group_number"]}
...
if save_forces:
    force_out_path = f"{out_dir}/force_sets_{slurm_array_job_id}-{slurm_array_task_id:>03}.json.gz"
