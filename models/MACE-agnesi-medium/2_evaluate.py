...
in_folder = f"2024-11-10-{model_name}-phononDB-LTC-FIRE_2SR_force0.0001_sym1e-05/"
...
df_mlp_print = df_mlp_filtered[["desc","SRME","SRE","kappa_TOT_ave",'DFT_kappa_TOT_ave','imaginary_freqs','errors']]
df_mlp_print["DFT_kappa_TOT_ave"] = df_mlp_print["DFT_kappa_TOT_ave"].apply(lambda x : x[0] if not pd.isna(x) else x)
df_mlp_print["kappa_TOT_ave"] = df_mlp_print["kappa_TOT_ave"].apply(lambda x : x[0] if not pd.isna(x) else x)
...
df_mlp_print = df_mlp_print[["desc","SRME","SRE","kappa_TOT_ave",'DFT_kappa_TOT_ave']]
